...
def initialize_train(model, batch_size, num_epochs, lr, weight_decay):
    ...
    def Params_norm(params, norm_params, y_min):

        params[:, 0] = (params[:, 0] - y_min.squeeze(1)) / norm_params.squeeze(1)
        params[:, 1] = params[:, 1] / norm_params.squeeze(1)
        params[:, 2] = -0.25 * (1 + torch.log10(params[:, 2]))
        params[:, 4] = -0.25 * (5 + torch.log10(params[:, 4]))
        params[:, 5] = (params[:, 5] - 0.1) / 0.216
    ...
    val_norm_data, Val_norm_value, val_y_min = preprocess(val_data)
    val_params_train = Params_norm(val_params_true, Val_norm_value, val_y_min)
    ...
    for i, epoch in enumerate(pbar):
        ...
        norm_data, data_norm_value, data_y_min = preprocess(data)
        optimizer.zero_grad()
        params = model(norm_data, attention_mask)
        params_train = Params_norm(params_true, data_norm_value, data_y_min )
